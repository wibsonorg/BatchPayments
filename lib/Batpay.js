const BigNumber = global.web3.BigNumber
...
class BP {
   ...
   * @param {number} delegate Id of the account to be the delegate
   */
  async bulkRegister (addresses, delegate) {
    ...
    const from = await this.getAddress(delegate);
    ...
    let tx = await this.bp.bulkRegister(bulkSize, tree.roothash, { from })
   ...
   * @param {number} delegate Id of the account to be the delegate
   */
  async claimBulkRegistrationId (bulk, address, accountId, delegate) {
    let i = accountId - bulk.smallestAccountId
    let proof = merkle.getProof(bulk.tree, i)
    proof = proof.map(x => x.v)
    const from = await this.getAddress(delegate);
    let tx = await this.bp.claimBulkRegistrationId(address, proof, accountId, bulk.bulkId, { from })
    let recp = await tx.receipt
   ...
   * @param {string} from Account to send the transaction.
   */
  async deposit (amount, accountId, from) {
    let options = from ? { from } : { from: await this.getAddress(accountId) };
    let nid = accountId

    let t1 = await this.st.approve(this.bp.address, amount, options)
    await t1.receipt

    let t2 = await this.bp.deposit(amount, accountId, options)
    await t2.receipt

    if (accountId == -1) {
      let log = findEvent(t2.logs, 'AccountRegistered')
      nid = log.accountId.toNumber()
      this.ids[nid] = log.accountAddress;
    }
  ...
  async registerPayment ({ fromAccountId, amount, unlockerFee, payeesAccountsIds, lockingKeyHash } = {}) {
    ...
    const from = await this.getAddress(fromAccountId);

    const tx = await this.bp.registerPayment(fromAccountId, amount, unlockerFee, payData, 0, 0, lockingKeyHash, 0, { from })

    const payIndex = (await this.bp.getPaymentsLength.call()) - 1
    this.payments[payIndex] = { amount, payData }
  ...
  async registerPaymentWithBulk (from, amount, fee, list, bulkList, lockingKeyHash) {
    let payData = getPayData(list)

    let nbulk = bulkList.length
    let tree = merkle.merkle(bulkList)

    let tx = await this.bp.registerPayment(from, amount, fee, payData, nbulk, tree.roothash, lockingKeyHash, 0)
    let recp = await tx.receipt
  ...
  async collect ({ delegate, slot, toAccountId, fromPaymentId, toPaymentId, amount, fee, address } = {}) {
    ...
    const from = await this.getAddress(delegate);
    let tx = await this.bp.collect(delegate, slot, toAccountId, toPaymentId, amount, fee, address, signature, { from })
    return tx
  }

  async freeSlot (delegate, slot) {
    const from = await this.getAddress(delegate);
    let tx = await this.bp.freeSlot(delegate, slot, { from })
    return await tx.receipt
  }

  async challenge_1 (delegate, slot, challenger) {
    const from = await this.getAddress(challenger);
    let tx = await this.bp.challenge_1(delegate, slot, challenger, { from })
    return await tx.receipt
  }

  async challenge_2 (delegate, slot, data) {
    const from = await this.getAddress(delegate);
    let tx = await this.bp.challenge_2(delegate, slot, data, { from })
    return await tx.receipt
  }

  async challenge_3 (delegate, slot, data, index, challenger) {
    const from = await this.getAddress(challenger);
    let tx = await this.bp.challenge_3(delegate, slot, data, index, { from })
    return await tx.receipt
  }

  async challenge_4 (delegate, slot, payData) {
    const from = await this.getAddress(delegate);
    let tx = await this.bp.challenge_4(delegate, slot, payData, { from })
    return await tx.receipt
  }

  async challenge_failed (delegate, slot) {
    const from = await this.getAddress(delegate);
    let tx = await this.bp.challenge_failed(delegate, slot, { from })
    return await tx.receipt
  }

  async challenge_success (delegate, slot, challenger) {
    const from = await this.getAddress(challenger);
    let tx = await this.bp.challenge_success(delegate, slot, { from })
    return await tx.receipt
  }
  ...
  async getCollectAmount (id, fromIndex, toIndex) {
    ...
    return v.reduce((acc, x) => {
      const added = (x >= fromIndex && x < toIndex) ? this.payments[x].amount : 0;
      return acc + added;
    }, 0);
  }
  ...
  getCollectData (id, fromIndex, toIndex) {
    ...
    return v.filter(x => x >= fromIndex && x < toIndex)
  }
  ...
  getPayData(payIndex) {
    return this.payments[payIndex].payData;
  }
  ...
  async showBalance(accountIds, log = () => false) {
    log('\nFetching Accounts');
    let accounts = []
    for (let i = 0; i < accountIds.length; i++) {
      const id = accountIds[i];
      const [addr, balance, lastCollectedPaymentId] = await this.getAccount(id)
      const tokenBalance = await this.tokenBalance(addr)
      accounts.push({
        'Account ID': id,
