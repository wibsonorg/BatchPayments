...
function hashCollect(delegate, toId, fromPayId, toPayId, amount, fee) {
    let hash = abi.soliditySHA3(
            ['uint32', 'uint32', 'uint32', 'uint32', 'uint64', 'uint64'], 
            [delegate, toId, fromPayId, toPayId, amount, fee]).toString("hex");

    return "0x"+hash;
}

function signCollect(account, delegate, toId, fromPayId, toPayId, amount, fee) {
    let hash = hashCollect(delegate, toId, fromPayId, toPayId, amount, fee);
