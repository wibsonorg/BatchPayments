...
async function newInstances() {
    ...
    Accounts.link('Merkle', merkle.address);
    let accounts = await Accounts.new();

    Payments.link('Merkle', merkle.address);
    Payments.link('Accounts', accounts.address);

    let payments = await Payments.new();

    BatPay.link('Merkle', merkle.address);
    BatPay.link('Accounts', accounts.address);
    BatPay.link('Payments', payments.address);
