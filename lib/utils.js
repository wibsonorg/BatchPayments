...
function hashCollect(delegate, toId, fromPayIndex, toPayIndex, amount, fee) {
    let hash = abi.soliditySHA3(
            ['uint32', 'uint32', 'uint32', 'uint32', 'uint64', 'uint64'], 
            [delegate, toId, fromPayIndex, toPayIndex, amount, fee]).toString("hex");

    return "0x"+hash;
}

function signCollect(account, delegate, toId, fromPayIndex, toPayIndex, amount, fee) {
    let hash = hashCollect(delegate, toId, fromPayIndex, toPayIndex, amount, fee);
