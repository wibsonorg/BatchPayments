...
var newAccount = new BigNumber(2).pow(256).minus(1);
...
class BP {
    ...
    async init() {
        this.unlockBlocks = (await this.bp.unlockBlocks.call()).toNumber();
        this.challengeBlocks = (await this.bp.challengeBlocks.call()).toNumber();
        this.challengeStepBlocks = (await this.bp.challengeStepBlocks.call()).toNumber();
    ...
    async bulkRegister(list) {
        ...
        for(let i = 0; i<list.length; i++) {
            this.ids[minId+i] = list[i];
        }
        
        
        return { tree, minId, bulkId, recp } ;
    }

    async claimId(bulk, addr, id) {
        let i = id - bulk.minId;
        let proof = merkle.getProof(bulk.tree, i);
        proof = proof.map(x=>x.v);
        let tx = await this.bp.claimId(addr, proof, id, bulk.bulkId); 
        let recp = await tx.receipt;
        let log = findEvent(tx.logs, 'Register');
        
        return [log.id, log.addr, recp ];
    }
    ...
    async deposit(amount, id, from) {
        ...
        if (id == -1) {
            let log = findEvent(t2.logs, 'Register');
            nid = log.id.toNumber();
        }
    ...
    async transfer(from, amount, fee, list, lock) {
        ...
        list.forEach(x=>{
            if (this.payList[x] == undefined) {
                this.payList[x] = [];
            }
            this.payList[x].push(payIndex);
        });
