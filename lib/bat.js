...
class BP {
  constructor (bp, token) {
    ...
    // This is a hacky way of returning a valid mock address even if the
    // accountId is not registered.
    // TODO Rethink the way accountIds and and addresses are handled.
    var accounts
    web3.eth.getAccounts(function (error, result) {
      if (error) {
        console.log(error)
    ...
    this.ids = new Proxy({}, {
      get: function (object, property) {
        return object.hasOwnProperty(property) ? object[property] : accounts[0]
      }
    })
  }
  ...
  async bulkRegister (list) {
    ...
    for (let i = 0; i < list.length; i++) {
      this.ids[smallestAccountId + i] = list[i]
    }
  ...
  async claimBulkRegistrationId (bulk, addr, id) {
    let i = id - bulk.smallestAccountId
  ...
  async registerPayment (from, amount, fee, list, lockingKeyHash) {
    let data = getPayData(list)
    let tx = await this.bp.registerPayment(from, amount, fee, data, 0, 0, lockingKeyHash, 0)
    await tx.receipt
  ...
  async collect (delegate, slot, to, fromId, toId, amount, fee, addr) {
    let signature = await signCollect(this.ids[to], this.bp.address, delegate, to, fromId, toId, amount, fee, addr)
  ...
  async showBalance () {
    for (let i = 0; i < 10; i++) {
      let [addr, balance, lastCollectedPaymentId] = await this.getAccount(i)
      let tb = await this.tokenBalance(this.ids[i])
      tb = tb.toString(10).padStart(9, ' ')
      balance = balance.toString(10).padStart(6, ' ')
      console.log(i + ':' + tb + ' ' + balance + ' ' + lastCollectedPaymentId + '\t' + addr)
    }
    console.log('----------------')
  }
}
